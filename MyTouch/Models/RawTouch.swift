...
extension RawTouch {
    ...
    struct Properties: OptionSet, Codable {
        ...
        static var force    = RawTouch.Properties(rawValue: 1 << 0)
        static var azimuth  = RawTouch.Properties(rawValue: 1 << 1)
        static var altitude = RawTouch.Properties(rawValue: 1 << 2)
        static var location = RawTouch.Properties(rawValue: 1 << 3) // For predicted Touches
    }
}
